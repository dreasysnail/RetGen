...
from dpr.utils.data_utils import ShardedDataIterator, read_data_from_json_files, read_data_from_wiki_to_json, Tensorizer
from dpr.utils.dist_utils import all_gather_list, is_main_process
...
class BiEncoderTrainer(object):
    ...
    def get_wiki_iterator(self, path: str, batch_size: int, shuffle=True,
                          shuffle_seed: int = 0,
                          offset: int = 0, upsample_rates: list = None) -> ShardedDataIterator:
        data_files = glob.glob(path)
    ...
    def get_dialog_iterator(self, path: str, batch_size: int, shuffle=True,
                            shuffle_seed: int = 0,
